...
class JuegoGeografía():
    ...
    def comprobacion_respuesta(self):
        ...
        respuesta_usuario = unicodedata.normalize('NFKD', respuesta_usuario).encode('ascii', 'ignore').decode('utf-8').lower()
        ...
        respuesta_usuario = re.sub(r'[^a-z0-9\s]', '', respuesta_usuario)
        respuesta_usuario = re.sub(r'\s+', ' ', respuesta_usuario).strip()
        ...
        if respuesta_usuario.strip().lower() == self.diccionario[self.pregunta].strip().lower():
            self.aciertos += 1
            ...
            if self.aciertos == 5: #Si llegamos a 5 respuestas correctas imprimimos has ganado
                ...
                boton_fin = Button(self.ventana, command=ventana.destroy,text="Cerrar Ventana",height= 3, width=15)
                boton_fin.grid(row=3, column=0, padx=60, pady=30, sticky="w")
        else:
...
def es_respuesta_correcta(respuesta_usuario, respuesta_correcta, umbral_similitud=0.8):
    respuesta_usuario = normalizar_texto(respuesta_usuario)
    respuesta_correcta = normalizar_texto(respuesta_correcta)
    longitud_maxima = max(len(respuesta_usuario), len(respuesta_correcta))
    distancia = distancia_levenshtein(respuesta_usuario, respuesta_correcta)
